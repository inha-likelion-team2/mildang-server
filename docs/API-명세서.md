...
화면 «온보딩_03»의 설문 3문항입니다.

| 필드 | 문항 | 값 | 예산 반영 |
|------|------|-----|----------|
| `noodle`·`bread`·`snack` | 평소 일주일에 얼마나 자주 드세요? | **`"0-1"` `"2-3"` `"4+"`** (필수) | 기본 추정치 |
| `portion` | 한 번 먹을 때 양은? | `SMALL`(조금) `NORMAL`(보통) `LARGE`(많이) | **×0.7 / ×1.0 / ×1.3** |
| `situation` | 가장 많이 먹는 상황은? | `MEAL` `SNACK` `LATE_NIGHT` `IRREGULAR` | **없음 (전부 ×1.0)** |

- `portion`·`situation`은 **선택**입니다. `portion`을 생략하면 `NORMAL`로 봅니다.
- ⚠ `situation`은 지금 예산을 바꾸지 않습니다. **저장만 해두고 나중에 AI(흥정 어조·팁·리포트)가 씁니다.** 화면에서는 받아주세요.
