...
**이 응답 하나로 메인 화면이 전부 그려집니다.**

**`challenge.status`로 어디로 갈지 갈립니다.**

| `status` | 뜻 | 화면 |
| --- | --- | --- |
| `ACTIVE` | 진행 중 | 메인보드 |
| `COMPLETED` | **마지막 날을 넘김** | **완주 리포트** — 같이 온 `challenge.id`로 `GET /challenges/{id}/report` |

챌린지를 **한 번도 안 만든 사람**만 **404 "진행 중인 챌린지가 없어요."**입니다(→ 온보딩으로).

> ⚠ 예전에는 완주도 404였습니다. 그래서 프론트가 «신규 유저»와 «방금 완주»를 구분하지 못하고 완주자도 온보딩으로 되돌려보냈습니다(FE 제보 2026-08-19). 완주 후엔 리포트를 부를 `challengeId`조차 알 수 없었고요. **이제 완주는 200 + `COMPLETED`로 옵니다.**
>
> 끝난 챌린지에 **기록·흥정·체크인 같은 쓰기는 여전히 404**입니다. 읽기(`current`)만 끝난 것을 보여줍니다.
>
> `COMPLETED`일 때 `tip`은 항상 `null`입니다 — 리포트로 넘어갈 참이라 오늘의 팁을 만들지 않습니다.
