...
✅ **2026-08-19 배포 완료. 전부 지금 쓰실 수 있습니다.** 실서버 검증 5/5 통과.

| 확인 | 결과 |
| --- | --- |
| `amount` SMALL / NORMAL / LARGE | 추정 **185 / 265 / 345** — 값마다 갈립니다 |
| 옛 이름 `portion=LARGE` | 345 (`amount`와 동일) |
| `weightKg` 50 / 65.5 / 90kg | 추정 **250 / 270 / 305** |
| `weightKg` 미전송 | 265 — 기존과 동일 |
| `situation` MEAL / LATE_NIGHT | 265 / 265 — 예산 미반영 |
...
[서버 이슈 #1](https://github.com/inha-likelion-team2/mildang-server/issues/1)에 올려주신 피드백입니다. **전부 배포돼 지금 동작합니다.** 배포본 + 실 AI로 유저플로우를 끝까지 돌려 확인했습니다.
...
**전부 배포 완료되어 지금 붙이실 수 있습니다.** `GET /v1/health`의 `codeVersion`으로 직접 확인하셔도 됩니다.
